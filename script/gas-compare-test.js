...
let printGas = function (size,r1,r2){
    const context = `${size.toString()}byte          ${r1.gasUsed}          ${r2.gasUsed} \n`;
    // console.log(context);
    writeInFile(context);
}
...
let WriteCode = async function (size){
    ...
    let opfdFactory = await ethers.getContractFactory("OptimizedFlatDirectory");
    let fdFactory = await ethers.getContractFactory("FlatDirectory");

    let opfd = await opfdFactory.deploy()
    await opfd.deployed()

    let fd = await fdFactory.deploy()
    await fd.deployed()
...
let run = async function(){
    ...
    const MaxSize = 300;
    for (let size=20;size<=MaxSize;size+=20){
